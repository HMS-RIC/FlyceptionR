...
measureROI <- function(img, mask, output, goodfr=F, fps=100){
  ...
  dat <- data.frame(x=(1:dim(img)[3])/fps, y=ROIdFF0)
  p <- ggplot(data=dat, aes(x=x, y=y)) + geom_point(size=0.25, alpha=0.5) +
    geom_smooth(method="loess", span = 0.2, level=0.95) +
    labs(x = "t (sec)", y = "dF/F (%)") +
    ylim(-5, 10)
  ...
  if(goodfr!=F){
    ROIdFF0[!goodfr] <- NA
    dat_goodfr <- data.frame(x=(1:dim(img)[3])/fps, y=ROIdFF0)
  p <- ggplot(data=dat, aes(x=x, y=y)) + geom_point(size=0.25, alpha=0.5) +
      geom_smooth(method="loess", span = 0.2, level=0.95) +
      labs(x = "t (sec)", y = "dF/F (%)") +
      ylim(-5, 10)
